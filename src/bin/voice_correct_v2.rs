...
//!   KIMI_API_KEY=sk-... cargo run --bin voice-correct-v2 --features sensevoice -- --engine sensevoice
...
#[derive(Parser)]
#[command(
    about = "Voice Correct V2 — hold right Cmd to dictate, tap+hold to correct, optional term correction"
)]
struct Args {
    ...
    /// Optional file with one correction term per line, e.g. names.
    #[arg(long)]
    terms_file: Option<PathBuf>,

    /// Lexicon used to build pronunciations for --terms-file.
    #[arg(long, default_value = "crates/homophone-replacer/assets/lexicon.txt")]
    terms_lexicon: PathBuf,
}
...
fn load_terms(path: &PathBuf) -> Result<Vec<String>, String> {
    let input = std::fs::read_to_string(path)
        .map_err(|e| format!("failed to read terms file {}: {e}", path.display()))?;
    ...
    if terms.is_empty() {
        return Err(format!("terms file {} is empty", path.display()));
    }

    Ok(terms)
}

fn build_term_correction(
    terms_path: &PathBuf,
    lexicon_path: &PathBuf,
) -> Result<TermCorrection, String> {
    let lexicon = Lexicon::from_path(lexicon_path)
        .map_err(|e| format!("failed to read lexicon {}: {e}", lexicon_path.display()))?;
    let terms = load_terms(terms_path)?;

    let mut owned_pairs = Vec::new();
    for term in terms {
        let pronunciation = pronounce_phrase(&lexicon, &term);
        if pronunciation.is_empty() || pronunciation == term {
            warn!(term = %term, "skipping term without usable pronunciation");
            continue;
        }
        owned_pairs.push((pronunciation, term));
    }

    if owned_pairs.is_empty() {
        return Err(format!(
            "no usable terms found in {} with lexicon {}",
            terms_path.display(),
            lexicon_path.display()
        ));
    }

    let refs = owned_pairs
        .iter()
    ...
    Ok(TermCorrection {
        lexicon,
        rules: ReplaceRuleSet::from_pairs(&refs),
    })
}

fn apply_term_correction_if_enabled(text: &str, correction: Option<&TermCorrection>) -> String {
    let Some(correction) = correction else {
        return text.to_string();
    };

    let corrected = replace_text(
        &correction.lexicon,
        &correction.rules,
    ...
    if corrected != text {
        info!(before = %text, after = %corrected, "term correction applied");
    }
...
unsafe extern "C-unwind" fn event_tap_callback(
    ...
    if event_type == CGEventType::FlagsChanged {
        ...
        if cmd_pressed && !was_down {
            ...
            // Other modifier changes while tracking cmd
            let is_rec = IS_RECORDING.load(Ordering::Relaxed);
            debug!(
                cmd_pressed,
                was_down,
                is_rec,
                flags = format_args!("0x{:04x}", device_flags),
                "FLAGS_CHANGED: no cmd transition"
            );
        }
    }
    event.as_ptr()
...
fn main() {
    ...
    let term_correction = match &args.terms_file {
        Some(terms_path) => match build_term_correction(terms_path, &args.terms_lexicon) {
            Ok(correction) => {
                info!(
                    terms = %terms_path.display(),
                    lexicon = %args.terms_lexicon.display(),
                    "term correction enabled"
                );
                Some(correction)
            }
            Err(err) => {
                error!(err = %err, "failed to enable term correction");
                None
            }
        },
        None => None,
    ...
    let _timer = unsafe {
        ...
        let block = RcBlock::new(move |_timer: NonNull<NSTimer>| {
            ...
            if SHOULD_STOP.swap(false, Ordering::Relaxed) && is_recording.get() {
                ...
                let spoken_for_typing =
                    apply_term_correction_if_enabled(&spoken, term_correction.as_ref());
                ...
                if mode == MODE_DICTATION {
                    ...
                    let mut spoken = apply_dictation_transforms(&spoken_for_typing, opts);
                    if opts.fullwidth_to_halfwidth
                    ...
                    match read_focused_text() {
                        ...
                        None => {
                            ...
                            info!(text = %spoken_for_typing, "field empty, typing instead");
                            type_text(&spoken_for_typing);
                            set_status_icon(&status_item, AppState::Idle, mtm);
                        }
                    }
